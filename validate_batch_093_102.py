...
# -*- coding: utf-8 -*-
"""
Validação do lote elev-093 a elev-102.
"""
...
BAD_TERMS = ["TÃ", "NÃ", "Ã§", "Ã£", "Ã¡", "Ã©", "Ã­", "Ã³", "Ãº"]
...
def main():
    ...
    if not CREATED_FILE.exists():
        errors.append("Arquivo batch_093_102_created_files.json não encontrado")
        print_report(errors)
        return 1
...
def print_report(errors):
    print("=" * 80)
    print("VALIDAÇÃO FINAL - LOTE ELEV-093 A ELEV-102")
    print("=" * 80)
    if errors:
        print(f"\nERROS ({len(errors)}):")
        for err in errors:
